...
on_peak_exp <- waiting_data %>%  #piping from waiting data
...
on_peak_exp_2 <- on_peak_exp %>% 
  arrange(desc(borough)) %>%
  mutate(prop = percent / sum(on_peak_exp$percent) *100) %>%
  mutate(ypos = cumsum(prop)- 0.5*prop )


#add % symbol to the end of the percent column obervations
on_peak_exp_2[ ,4] <- paste0(as.matrix(on_peak_exp_2[, 4]), '%')
